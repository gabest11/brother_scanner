...
class ScannerButton(ButtonEntity):
    def __init__(self, hass, ip, entry_id):
        ...
        self._attr_name = f"Fetch Snapshot ({ip})"
        self._attr_unique_id = f"{entry_id}_fetch_snapshot"
...
async def fetch_and_return_jpeg(hass, ip: str, filename: str, lock: asyncio.Lock):
    ...
    async with lock:
        try:
            ...
            await hass.async_add_executor_job(lambda: open(filename, "wb").write(jpeg_bytes))

            _LOGGER.info("Snapshot saved: %s", filename)
            
            hass.bus.async_fire(f"{DOMAIN}_snapshot_saved", {"ip": ip, "filename": filename})
