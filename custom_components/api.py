...
async def async_soap_request(session: aiohttp.ClientSession, url: str, xml: str) -> bytes:
...
async def scan_jpeg(ip: str) -> bytes:
    ...
    async with aiohttp.ClientSession() as session:
        # 1. Ensure idle
        state_xml = GET_SCANNER_STATUS_XML.format(url=url, msgid=make_uuid(), fromid=make_uuid())
        resp_bytes = await async_soap_request(session, url, state_xml)
        m = re.search(br"<wscn:ScannerState>(.*?)</wscn:ScannerState>", resp_bytes)
        ...
        jid = re.search(br"<wscn:JobId>(\d+)</wscn:JobId>", resp_bytes)
        jtok = re.search(br"<wscn:JobToken>(.*?)</wscn:JobToken>", resp_bytes)
        if not jid or not jtok:
            raise Exception("Failed to create scan job")
        jobid, jobtoken = jid.group(1).decode(), jtok.group(1).decode()

        # 3. Retrieve image
        xml = RETRIEVE_IMAGE_XML.format(url=url, msgid=make_uuid(), jobid=jobid, jobtoken=jobtoken)
        resp_bytes = await async_soap_request(session, url, xml)
